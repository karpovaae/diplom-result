...
//ВКМ
	...
	Пока Результат.Следующий() Цикл
		...
		Если СоздатьРеализацию Тогда
			...
			Иначе
				Реализация = ОбновитьРеализацию(Результат, Дата);
				РеализацииСтруктура.Вставить("Договор", Результат.Договор);
...
Функция ОбновитьРеализацию(Результат, Дата)
	
//ВКМ
	
	Реализация = Результат.Реализация.ПолучитьОбъект();
	Реализация.Дата = Дата;
	Реализация.Договор = Результат.Договор;
	Реализация.Контрагент = Результат.Контрагент;
	Реализация.Организация = Результат.Организация;
	Реализация.ВыполнитьАвтозаполнение();
	Реализация.Основание = Документы.ЗаказПокупателя.ПустаяСсылка();
